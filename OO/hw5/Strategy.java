...
public class Strategy {
    ...
    private boolean canInElevator(int curWeight, String fromFloor, int direction) {
        ...
        synchronized (requestqueue) {
            for (PersonRequest request : requestqueue.getRequests()) {
                if (request.getFromFloor().equals(fromFloor)
                        && Strategy.getDirection(request) == direction
                        && request.getWeight() + curWeight <= 400) {
                    return true;
                }
            }
            return false;
